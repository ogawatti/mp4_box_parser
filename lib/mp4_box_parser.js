...
module.exports = class Mp4BoxParser {
  ...
  static parseBox_stsd(size, type, fd, position) {
    ...
    for (let i = 1; i <= box.entryCount; i++) {
      let entryBox = this._readSizeAndType(fd, position)
      box[entryBox.type] = this.parseBox_xxxx(entryBox.size, entryBox.type, fd, position)
  ...
  static parseBox_avc1(size, type, fd, position) {
    let box = this._readSizeAndType(fd, position)
    let endPosition = position + size
    let buffer
    position += 8

    buffer = this._readBytes(fd, position, 6)
    //box.reserved = this._buffer2integer(buffer)
    position += 6

    buffer = this._readBytes(fd, position, 2)
    box.dataReferenceIndex = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 2)
    box.version = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 2)
    box.revisionLevel = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 4)
    box.vendor = this._buffer2integer(buffer)
    position += 4

    buffer = this._readBytes(fd, position, 4)
    box.temporalQuality = this._buffer2integer(buffer)
    position += 4

    buffer = this._readBytes(fd, position, 4)
    box.spacialQuality = this._buffer2integer(buffer)
    position += 4

    buffer = this._readBytes(fd, position, 2)
    box.width = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 2)
    box.height = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 4)
    box.horizontalResolution = this._buffer2integer(buffer)
    position += 4

    buffer = this._readBytes(fd, position, 4)
    box.verticalResolution = this._buffer2integer(buffer)
    position += 4

    buffer = this._readBytes(fd, position, 4)
    box.entryDateSize = this._buffer2integer(buffer)
    position += 4

    buffer = this._readBytes(fd, position, 2)
    box.framePerSample  = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 32)
    //box.compressorName = this._buffer2string(buffer)
    position += 32

    buffer = this._readBytes(fd, position, 2)
    box.bitDepth = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, 2)
    box.colorTableIndex = this._buffer2integer(buffer)
    position += 2

    // avcC box parse
    let avcC = this._readSizeAndType(fd, position)
    box.avcC = this.parseBox_avcC(avcC.size, avcC.type, fd, position)

    return box
  }

  static parseBox_avcC(size, type, fd, position) {
    let box = { size: size, type: type }
    let buffer
    position += 8

    buffer = this._readBytes(fd, position, 1)
    box.configurationVersion = this._buffer2integer(buffer)
    position += 1

    buffer = this._readBytes(fd, position, 1)
    box.avcProfileIndication = this._buffer2integer(buffer)
    position += 1

    buffer = this._readBytes(fd, position, 1)
    box.profileCompatibility = this._buffer2integer(buffer)
    position += 1

    buffer = this._readBytes(fd, position, 1)
    box.avcLevelIndication = this._buffer2integer(buffer)
    position += 1

    buffer = this._readBytes(fd, position, 1)
    /*
    if (buffer[0] === 3 || (buffer[0] >> 2 === 0x3F)) {
      box.nalUnitSize = 4
    }
    */
    box.nalUnitSize = 4  // 3 + 1
    position += 1

    // SPS : SequenceParameterSet
    buffer = this._readBytes(fd, position, 1)
    box.spsEntryCount = buffer[0] === 1 ? buffer[0] : buffer[0] - 0xE0  // 1
    position += 1

    buffer = this._readBytes(fd, position, 2)
    box.spsSize = this._buffer2integer(buffer)
    position += 2

    box.sps = this.parseSPS(box.spsSize * box.spsEntryCount, fd, position)
    position += box.spsSize * box.spsEntryCount

    // TODO PPS : PictureParameterSet
    buffer = this._readBytes(fd, position, 1)
    box.ppsEntryCount = buffer[0]
    position += 1

    buffer = this._readBytes(fd, position, 2)
    box.ppsSize = this._buffer2integer(buffer)
    position += 2

    buffer = this._readBytes(fd, position, box.ppsSize * box.ppsEntryCount)
    //box.ppsData = buffer
    position += box.ppsSize * box.ppsEntryCount

    return box
  }

  static parseSPS(size, fd, position) {
    let sps = {}
    let buffer
    ...
    buffer = this._readBytes(fd, position, 1)
    sps.profileIndication = buffer[0]
    position += 1

    buffer = this._readBytes(fd, position, 1)
    let string = ('00000000' + buffer[0].toString(2)).slice(-8)
    sps.constraintSet0Flag = parseInt(string[0])
    sps.constraintSet1Flag = parseInt(string[1])
    sps.constraintSet2Flag = parseInt(string[2])
    sps.constraintSet3Flag = parseInt(string[3])
    sps.constraintSet4Flag = parseInt(string[4])
    sps.constraintSet5Flag = parseInt(string[5])
    position += 1

    buffer = this._readBytes(fd, position, 1)
    sps.levelIndication = buffer[0]
    position += 1
  ...
  static parseBox_uuid(size, type, fd, position) {
    ...
    let box = {}
    box[uuid] = { size: size, type: type, uuid: uuid }

    let stlength = size < this.maxSize ? 8 : 16
    let parsedBox = this.parseBox(fd, position + stlength + 16, position + size)
    Object.assign(box[uuid], parsedBox)
    return box
  }
